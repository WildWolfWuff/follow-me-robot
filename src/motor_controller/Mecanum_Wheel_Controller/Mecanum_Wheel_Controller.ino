...
{
  left_front_wheel_speed = ((speedX + speedY + speedZ) / 3) * MAX_SPEED;
  left_rear_wheel_speed = ((speedX - speedY + speedZ) / 3) * MAX_SPEED;
  right_front_wheel_speed = ((speedX - speedY - speedZ) / 3) * MAX_SPEED;
  right_rear_wheel_speed = ((speedX + speedY - speedZ) / 3) * MAX_SPEED;
}
...
void move()
{
  if (abs(left_front_wheel_speed - LeftFrontWheel.speed()) > tolerance)
  {
    LeftFrontWheel.setSpeed(left_front_wheel_speed);
    // Serial.println("Left Front Wheel: " + String(left_front_wheel_speed));
  }
  if (abs(left_rear_wheel_speed - LeftRearWheel.speed()) > tolerance)
  {
    LeftRearWheel.setSpeed(left_rear_wheel_speed);
    // Serial.println("Left Rear Wheel: " + String(left_rear_wheel_speed));
  }
  if (abs(right_front_wheel_speed - RightFrontWheel.speed()) > tolerance)
  {
    RightFrontWheel.setSpeed(right_front_wheel_speed);
    // Serial.println("Right Front Wheel: " + String(right_front_wheel_speed));
  }
  if (abs(right_rear_wheel_speed - RightRearWheel.speed()) > tolerance)
  {
    RightRearWheel.setSpeed(right_rear_wheel_speed);
